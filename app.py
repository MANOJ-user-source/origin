...
def chat_api():
    """Handle chat queries about uploaded stories"""
    if 'token' not in session:
        return jsonify({'error': 'Authentication required'}), 401
    ...
    try:
        ...
        conn = get_db_connection()
        stories = conn.execute(
            'SELECT title, content FROM stories WHERE user_id = ? ORDER BY created_at DESC',
            (user_id,)
        ).fetchall()
        conn.close()
        ...
        # Simple RAG-like response based on story content
        # In a real app, this would use vector embeddings and semantic search
        relevant_stories = []
        answer = "Based on your uploaded stories:\n\n"
        ...
        if relevant_stories:
            answer += f"I found {len(relevant_stories)} relevant story/stories:\n\n"
            for i, story in enumerate(relevant_stories, 1):
                answer += f"{i}. **{story['title']}**: {story['content']}\n\n"
        else:
            answer += "I couldn't find specific stories directly related to your question. Here are your available stories:\n\n"
            for i, story in enumerate(stories, 1):
                answer += f"{i}. **{story['title']}**: {story['content'][:100]}...\n\n"
        
        return jsonify({
            'answer': answer,
            'sources': [story['title'] for story in relevant_stories] if relevant_stories else [story['title'] for story in stories]
        })
